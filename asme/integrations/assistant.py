...
def _setting(name: str, env_name: str) -> str:
    """Prefer the typed app settings; fall back to the environment outside an app context."""
    try:
        from flask import current_app

        return getattr(current_app.config["SETTINGS"], name, "") or os.environ.get(env_name, "")
    except Exception:
...
def run_assistant(user_message: str, history: list | None = None, max_steps: int = 6) -> dict:
    ...
    api_key = _setting("anthropic_api_key", "ANTHROPIC_API_KEY")
    if not api_key:
        return {
    ...
    for _ in range(max_steps):
        resp = client.messages.create(
            model=_setting("anthropic_model", "ANTHROPIC_MODEL") or "claude-haiku-4-5-20251001",
            max_tokens=1024,
